...
class PDFJSInterface extends EventEmitter {
  ...
  static getInstance(iframe) {
    ...
    // 检查是否已有实例
    if (PDFJSInterface.instances.has(iframeElement)) {
      const existingInstance = PDFJSInterface.instances.get(iframeElement);
    ...
    PDFJSInterface.instances.set(iframeElement, instance);
    return instance;
  }
